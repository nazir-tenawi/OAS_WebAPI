...
using Oas.BusinessTracker.DataAccess.Common;
...
namespace Oas.AttendanceTracking.DataAccess
{
    public class LeaveDataAccess : BaseDatabaseHandler, ILeave
    {
        public List<LeaveModel> GetLeaveByCompanyId(int companyId)
        {
            ...
            string sql = @"SELECT la.* ,eu.PortalUserId,uc.FullName as EmployeeName,AP.FullName ApprovedBy from LeaveApplication as la
        ...
        {
            string err = string.Empty;
            string sql = @"SELECT la.* ,eu.PortalUserId,eu.EmployeeName,AP.FullName ApprovedBy from LeaveApplication as la
                            Left JOIN Employee eu on eu.id= la.EmployeeId
                            Left JOIN UserCredentials uc on uc.id= eu.PortalUserId
                            LEFT JOIN UserCredentials AP ON LA.ApprovedById=AP.Id
